...
JSON        a python 3, invoked inline via a heredoc (bin/focus-kit:219)
...
in an `ok` line or a `die`: `check_parses` (750), `check_install` (759),
`check_idempotent` (1168), `check_frontmatter` (1185), `check_no_em_dash`
(1224), `check_dogfood` (1236). They are the six checks of
...
(`bin/focus-kit:1286`), and `--help` prints the script's own header comment
...
| Touch a project-owned file | Written, never. Read, twice. `install_repo` tests whether `docs/00-Product.md` exists, to choose which closing message to print (`bin/focus-kit:362`), and the Manual citation pass of `doctor` reads `CLAUDE.md`, `docs/00` to `06` and everything under `docs/adr/`, line by line, to name a citation the next `update` can invalidate. Reading is not what ADR-0002 forbids, which is an edit: the fix for what the pass finds is a hand edit, in a file the CLI leaves alone. `work/` is read by neither. |
