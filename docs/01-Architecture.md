...
JSON        a python 3, invoked inline via a heredoc (bin/focus-kit:385)
...
(`bin/focus-kit:1652`), and `--help` prints the script's own header comment
...
| Touch the user's home | Only `graphify install --platform <platform>`, once per Host that `kit_hosts` names, and only when `global_skill_state` says that Host's Global skill is `missing`, `older` or `unknown` (`bin/focus-kit:221`). What it writes is graphify's, not the kit's: the skill, its `references/` and the Skill stamp, under the directory `kit_hosts` gives that Host, plus whatever that platform's own installer registers beside it. A skill `newer` than the package is never touched, because the installer would downgrade it, and an installer that fails for a Host is a `warn` naming what would have placed it. |
...
* **A function whose output is captured never calls `die`; it returns
  ...
  (`bin/focus-kit:73`, `bin/focus-kit:378`); `without_cr`, `fingerprint` and
