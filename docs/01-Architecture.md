...
JSON        a python 3, invoked inline via a heredoc (bin/focus-kit:439)
Content     markdown: 4 skills, 3 manuals, 11 templates, 3 config fragments
...
(`bin/focus-kit:1726`), and `--help` prints the script's own header comment
...
The four message shapes are `say`, `ok`, `warn`, `die` (`bin/focus-kit:57`).
...
skills/initialize/templates/      CLAUDE.md, .github/ and docs/, mirrors the target tree
...
.github/copilot-instructions.md   this repository's own Host instructions file
```
...
mirrors the target's (`CLAUDE.md`, `docs/00` to `06`, `docs/adr/` and, since
`copilot-reads-the-project-rules`, the Host instructions file of every other
host) rather than the kit's. That is deliberate: the template tree and the
tree it produces look the same, so a change to one is obvious in the other.
It is also what lets `doctor` name a path it holds no literal for: the
mirroring makes the template's own path the path the target takes, so
reading the tree answers where the file goes (§3, `doctor`).

`.github/copilot-instructions.md` at the root here is not a source of
anything and not a copy of one. This repository is its own target and
project-owned files are `/initialize`'s to write, so it was written by hand
from the template, once, and no command of the kit's touches it again
(`ADR-0002`).
...
| Touch a project-owned file | Written, never. Read, four times. `install_repo` tests whether `docs/00-Product.md` exists, to choose which closing message to print (`bin/focus-kit:605`); the Manual citation pass of `doctor` reads `CLAUDE.md`, `docs/00` to `06` and everything under `docs/adr/`, line by line, to name a citation the next `update` can invalidate; `doctor` reads the Manual language, the one project-owned file no person is expected to type, written by `/initialize` at the step where it asks the documentation language; and `doctor`'s presence loop tests whether each Host instructions file (`docs/03-Domain.md`) is there, at the path the Template tree gives it. Reading is not what ADR-0002 forbids, which is an edit: the fix for what the pass finds is a hand edit, in a file the CLI leaves alone. `work/` is read by neither. |
...
| Touch the user's home | Only `graphify install --platform <platform>`, once per Host that `kit_hosts` names, and only when `global_skill_state` says that Host's Global skill is `missing`, `older` or `unknown` (`bin/focus-kit:222`). What it writes is graphify's, not the kit's: the skill, its `references/` and the Skill stamp, under the directory `kit_hosts` gives that Host, plus whatever that platform's own installer registers beside it. A skill `newer` than the package is never touched, because the installer would downgrade it, and an installer that fails for a Host is a `warn` naming what would have placed it. |
...
* `set -euo pipefail` at the top (`bin/focus-kit:44`). An unhandled failure
...
* **A function whose output is captured never calls `die`; it returns
  ...
  (`bin/focus-kit:74`, `bin/focus-kit:432`); `without_cr`, `fingerprint` and
