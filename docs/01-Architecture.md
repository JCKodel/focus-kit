...
| `selftest` | 842 | The verify command: creates the scratch repository, calls the six checks in order, removes the scratch through a `trap ... EXIT`. |
...
in an `ok` line or a `die`: `check_parses` (516), `check_install` (525),
`check_idempotent` (742), `check_frontmatter` (759), `check_no_em_dash`
(798), `check_dogfood` (810). They are the six checks of
...
(`bin/focus-kit:860`), and `--help` prints the script's own header comment
