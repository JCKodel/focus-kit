...
in an `ok` line or a `die`: `check_parses` (590), `check_install` (599),
`check_idempotent` (911), `check_frontmatter` (928), `check_no_em_dash`
(967), `check_dogfood` (979). They are the six checks of
...
(`bin/focus-kit:1029`), and `--help` prints the script's own header comment
...
| Touch a project-owned file | Never. The single read is `[ -f "$target/docs/00-Product.md" ]`, to choose which closing message to print (`bin/focus-kit:359`). |
