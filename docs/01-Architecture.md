...
| `selftest` | 779 | The verify command: creates the scratch repository, calls the six checks in order, removes the scratch through a `trap ... EXIT`. |
...
in an `ok` line or a `die`: `check_parses` (500), `check_install` (509),
`check_idempotent` (679), `check_frontmatter` (696), `check_no_em_dash`
(735), `check_dogfood` (747). They are the six checks of
...
(`bin/focus-kit:797`), and `--help` prints the script's own header comment
