...
JSON        a python 3, invoked inline via a heredoc (bin/focus-kit:297)
...
(`bin/focus-kit:1427`), and `--help` prints the script's own header comment
...
The four message shapes are `say`, `ok`, `warn`, `die` (`bin/focus-kit:49`).
...
| Situation | How |
|---|---|
| Read the kit's own files | Relative to `KIT_DIR`, resolved from the script's real path through symlinks (`bin/focus-kit:41`). Never relative to the caller's working directory. |
...
| Touch a project-owned file | Written, never. Read, twice. `install_repo` tests whether `docs/00-Product.md` exists, to choose which closing message to print (`bin/focus-kit:440`), and the Manual citation pass of `doctor` reads `CLAUDE.md`, `docs/00` to `06` and everything under `docs/adr/`, line by line, to name a citation the next `update` can invalidate. Reading is not what ADR-0002 forbids, which is an edit: the fix for what the pass finds is a hand edit, in a file the CLI leaves alone. `work/` is read by neither. |
...
| Touch the user's home | Only `graphify install --platform claude`, and only when `global_skill_state` says the Global skill is `missing`, `older` or `unknown` (`bin/focus-kit:177`). What it writes is graphify's, not the kit's: the skill, its `references/` and the Skill stamp, plus a section appended to `~/.claude/CLAUDE.md` when that file does not mention graphify yet. A skill `newer` than the package is never touched, because the installer would downgrade it. |
...
* `set -euo pipefail` at the top (`bin/focus-kit:36`). An unhandled failure
...
* **A function whose output is captured never calls `die`; it returns
  ...
  (`bin/focus-kit:66`, `bin/focus-kit:290`); `without_cr`, `fingerprint` and
