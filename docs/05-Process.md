...
| First target (`~/Downloads/vaulted`) | installed and initialized at the delivery's version while milestone 2 is open; nothing committed there, ever, because it is a clone of someone else's repository, and what a command stages there is undone with `git reset` before the record is written. This row leaves with milestone 2 (`docs/03-Domain.md`, First target). | `focus-kit update ~/Downloads/vaulted` |
