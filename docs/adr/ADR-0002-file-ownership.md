...
* **Kit-owned**, written with `copy_tree` (`bin/focus-kit:89`): the
...
* **Project-owned**, never written by the CLI at all: `docs/00` to `06`,
  ...
  closing message to print (`bin/focus-kit:174`).
* **Merged**, written with `merge_json` (`bin/focus-kit:95`): `.mcp.json`
