...
| Host | (the kit's own prose; `docs/01-Architecture.md` §4) | The agent tool that reads a target's Commands. Two of them since `copilot-port`: Claude Code, which reads `.claude/skills/<name>/SKILL.md`, and GitHub Copilot, which reads a Ported command. One Command, one text, one file per host, because the second file is generated from the first and never written beside it (`ADR-0007`). No Command's text names a host: what is host-specific is the substitution list of the transform, in `bin/focus-kit` and nowhere else. |
...
| Ported command | (written by `install_repo`, one file per Command per Host other than Claude Code) | The Host's copy of a Command, generated at install from `skills/<name>/SKILL.md`: the same body word for word, with a fixed substitution list replacing what names Claude Code alone, the argument placeholder, the tool that asks, the new session, the host and the repository instructions file. Kit-owned, listed in the Manifest, carrying the Kit-owned banner and the License notice, so the next `update` overwrites an edit made in a target. Its directory, its name, its extension and its frontmatter keys are the host's to declare and the run's to read, never this table's to pin. Added by `copilot-port`. |
...
| Project-owned | (never written by the CLI) | A file only `/initialize` and the people working in the target may touch: `docs/00` to `06`, `CLAUDE.md`, `docs/adr/`, `work/`. The CLI never writes one. It reads two of them: `install_repo` checks whether `docs/00-Product.md` exists, to decide which next step to print (`bin/focus-kit:362`), and `doctor`'s Manual citation pass reads `CLAUDE.md`, `docs/00` to `06` and everything under `docs/adr/` line by line, `work/` excepted. Reading is not what `ADR-0002` forbids, which is the write. |
...
| Target repository | `target` | The repository the kit is installed into. Inside the CLI it is always an absolute path (`bin/focus-kit:292`). |
