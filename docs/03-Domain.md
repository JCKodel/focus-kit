...
| As written | (the phrase `written here once and asked as written`, `skills/initialize/SKILL.md`) | What a fixed text of `/initialize` keeps when it reaches the person. Five texts: the Language question, the four Practice questions, the Proof tool question, the two-patterns disclaimer and the no screen line. Each is conversation and not document, so it is asked or said in the Conversation language, with every statement, every option and their order carried over and nothing added or dropped; the kit's own terms, paths and file names stay as the skill's Language section already keeps them in a document. The English text in the skill is the source, and a record that quotes what was asked is compared with it statement by statement, never byte by byte. In an English conversation the two are the same bytes. First occurrence: the scratch of `practice-questions-all-yes`, 2026-09-17, where one session asked the Language question in English and said the other four texts in Portuguese, and neither was wrong against the file (`asked-as-written-says-which-language`). The rule is what the Language section says; what a run does with it is measured, and four of the five followed it the day the rule was written. The fifth, the no screen line, came out in English because the section writes it in backticks, and the Graph confirmation of `docs/manuals/graphify.md`, whose own rule uses the same phrase, kept its three option labels in English for the same reason. A text set off as a literal is the open case, and it is `as-written-covers-a-literal`. |
