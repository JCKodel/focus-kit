...
| Git strategy | `**Strategy.**`, the first line of `docs/05-Process.md` §7 | Which of three ways a repository works with git: a worktree per delivery, a branch per slug, or none. Chosen once, by `/initialize`, and written into the git slot, which is prose today and which every run words for itself. `/propose` and `/apply` read it and act on it, and none of the three lets the agent commit or merge on its own (`docs/00-Product.md`, Building it). Added by `git-strategy-is-asked`. |
