...
* **A terminal line picks one of four shapes.** `say`, `ok`, `warn`, `die`
  (`bin/focus-kit:57`). Never a bare `echo` or `printf` for a message a
...
* **A `warn` says what to do next.** "uv missing" is half a message; the line
  adds the command that puts it right. Compare `bin/focus-kit:667`, and every
...
* **The help text is the script's own header.** `--help` prints it through
  `awk` (`bin/focus-kit:1742`), by a rule and not a range: the shebang is
...
* **The header names every path `install` writes into a target.** Item 2 of
  ...
  (`bin/focus-kit:557`), so a check comparing the `ok` output with the header
