...
Implement `work/${input:arguments}.md` in this session, completely.
...
before this one,
say `this session already ran <command>; open a new one (a new chat, or a new
terminal) and type /apply ${input:arguments}` and stop, before Read first: nothing
...
this session is not in the one for `${input:arguments}`, say the strategy, where you
...
1. `work/${input:arguments}.md`: the delivery. It is the scope; do not widen it.
...
1. Write into `work/${input:arguments}.md` what happened: what diverged from the
