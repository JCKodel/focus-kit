...
`multiple choice question` carrying all four, each explained in a line. The person
...
`multiple choice question`, the question `How is a screen proven? No file names the
...
`multiple choice question`. This is not a Practice and FOCUS has no answer to it, so
